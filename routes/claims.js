...
router.post('/notify-owner', authenticateToken, [
    ...
    try {
        ...
        //This part handles the found item that will be linked to the lost item notification.
        //The finder already has a registered found item → use that item.
        let foundItem = null;
        if (found_item_id) {
        ...
        //This code checks whether the exact same lost item and found item already have a claim/notification.
...
//This route handles the situation where the claimer confirms that they have received/claimed the item back.
router.patch('/:id/claimer-confirm', authenticateToken, async (req, res) => {
    try {
        ...
        if (claim.status === 'finder_marked') {
            //it decides whether the claim is:completed immediately, because the finder has already confirmed, or only marked by the claimer, because the finder has not confirmed yet.
            // Both sides confirmed ->  auto-complete and close item
...
router.post('/verify-request', authenticateToken, [
    ...
    try {
        ...
        const { found_item_id, location } = req.body;
        ...
        // This is the part that actually creates the claim after the location verification. The important thing is that it handles both outcomes: approved and rejected.
...
// it is the route that supplies the finder-side card with the information about the claimant and the current claim status.
...
// Get the logged-in user's claims and show finder details
...
//This route returns the IDs of found items that are already locked/unavailable for new claims  
//because they have an active/completed claim or are closed.
//The frontend uses these IDs to prevent other users from requesting those items.
