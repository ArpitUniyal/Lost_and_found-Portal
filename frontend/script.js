...
async function refreshNotificationHiddenFound() {
    notificationHiddenFoundIds = new Set();
    ...
    try {
        ...
        const res = await fetch(
            `${API_BASE}/claims/found/notification-hidden`,
            {
                headers: {
        ...
        const ids =
            body.data && Array.isArray(body.data.found_item_ids)
                ? body.data.found_item_ids
                : [];

        ids.forEach(id =>
            notificationHiddenFoundIds.add(Number(id))
        );
...
let notificationHiddenFoundIds = new Set();
...
document.getElementById('verify-claim-form').addEventListener('submit', async(e) => {
    ...
    try {
        ...
        const res = await fetch(`${API_BASE}/claims/verify-request`, {
            ...
            body: JSON.stringify({ foundItemId: foundId, location })
        });
        const body = await res.json();
...
document
    ...
    document.querySelectorAll('.hero-buttons .btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // ✅ Scroll control when navigating from hero
            if (section === 'home') {
...
async function loadFinderPendingClaims() {
    ...
    try {
        ...
        for (const c of claims) {
            ...
            if (c.lost_item_id) {
                myFinderClaimByLost.set(c.lost_item_id, {
    id: c.id,
    status: c.status,
...
document.addEventListener('click', async (e) => {
    const finderReturnedBtn =
        e.target.closest('.finder-returned') ||
        e.target.closest('.lost-finder-returned');

    const claimerConfirmBtn =
        e.target.closest('.claimer-confirm-btn') ||
        e.target.closest('.lost-claimer-confirm');

    if (!finderReturnedBtn && !claimerConfirmBtn) return;
    ...
    try {
        ...
        if (finderReturnedBtn) {
            const claimId = finderReturnedBtn.getAttribute('data-claim-id');

            const response = await fetch(
                `${API_BASE}/claims/${claimId}/finder-returned`,
                {
                    method: 'PATCH',
                    headers: {
                        'Authorization': `Bearer ${token}`
            ...
            if (response.ok) {
                showToast(
                    data.message || 'Item return recorded',
                    'success'
                );

                // Refresh both lost and found sections
                ...
                showToast(
                    data.message || 'Failed to record item return',
                    'error'
                );
            }
        ...
        // Claimer confirms that the item was claimed back
        if (claimerConfirmBtn) {
            const claimId = claimerConfirmBtn.getAttribute('data-claim-id');

            const response = await fetch(
                `${API_BASE}/claims/${claimId}/claimer-confirm`,
                {
                    method: 'PATCH',
                    headers: {
                        'Authorization': `Bearer ${token}`
           ...
           if (response.ok) {
    showToast(
        data.message || 'Claimed back recorded',
        'success'
    );

    // Immediately update the owner's local claim state
    for (const [lostId, claim] of myClaimByLost.entries()) {
        if (claim.id === Number(claimId)) {
            myClaimByLost.set(lostId, {
                ...claim,
                status: 'completed'
            });
            break;
    ...
    await loadFinderPendingClaims();
                ...
                showToast(
                    data.message || 'Failed to record claimed back',
                    'error'
                );
            }
        }
...
function updateAuthUI(isLoggedIn) {
    if (isLoggedIn) {
        ...
        if (!finderClaimsInterval) {
            finderClaimsInterval = setInterval(async () => {
    if (!currentUser) return;

    try {
        await refreshLockedFound();
        await refreshNotificationHiddenFound();
        await refreshMyActiveClaims();
        await loadFinderPendingClaims();
...
function displayLostItems(items) {
    ...
    container.innerHTML = items.map(item => {
        ...
        const showFinderReturned =
    !!finderClaim &&
    finderClaim.finder_id === currentUser.id &&
    ['approved', 'claimer_marked'].includes(finderClaim.status);
        const showOwnerClaimedBack = !!ownerClaim && ['approved','finder_marked'].includes(ownerClaim.status);
        return `
        <div class="item-card">
            ${item.image_url ? `<div class=\"item-image\"><img src=\"${escapeHtml(getImageUrl(item.image_url))}\" alt=\"${escapeHtml(item.item_name)}\"></div>` : ''}
...
if (my && my.status === 'approved') {
    return `<button class="btn btn-secondary" disabled>Waiting</button>`;
}

if (my && my.status === 'finder_marked') {
    return `<button class="btn btn-primary claimer-confirm-btn" data-claim-id="${my.id}">Item Claimed Back</button>`;
}

if (my && my.status === 'claimer_marked') {
    return `<button class="btn btn-secondary" disabled>Waiting</button>`;
}
    ...
    if (myActiveFound.length === 0) {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/claims/notify-owner`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                body: JSON.stringify({ lostItemId: Number(lostId) })
            });
            const body = await res.json();
            if (res.ok) {
...
document.getElementById('notify-owner-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const res = await fetch(`${API_BASE}/claims/notify-owner`, {
            ...
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
            body: JSON.stringify({
    lostItemId: Number(lostId),
    foundItemId: Number(foundId)
})
        });
        const body = await res.json();
        if (res.ok) {
...
if (claim && claim.lost_item_id) {
    ...
    myFinderClaimByLost.set(
        Number(claim.lost_item_id),
        { id: claim.id, status: claim.status || 'pending' }
    );
}
            displayLostItems(allLostItems);
            loadFinderPendingClaims().catch(() => {});
...
function formatDate(dateString) {
    ...
    if (isNaN(date.getTime())) {
        ...
        if (!isNaN(alt.getTime())) {
            return alt.toLocaleDateString('en-US', options);
        }
